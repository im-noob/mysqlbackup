...
class InitParam{
    function doInitParamAndGetData($DB_USER,$DB_PASSWORD, $DB_NAME, $DB_HOST, $BACKUP_DIR, $TABLES, $CHARSET, $GZIP_BACKUP_FILE, $BATCH_SIZE){
        echo ($DB_USER); echo($DB_PASSWORD); echo($DB_NAME); echo($DB_HOST); echo($BACKUP_DIR); echo($TABLES); echo($CHARSET); echo($GZIP_BACKUP_FILE); echo($BATCH_SIZE);
        define("DB_USER", $DB_USER);
        define("DB_PASSWORD", $DB_PASSWORD );
        define("DB_NAME", $DB_NAME);
        define("DB_HOST", $DB_HOST);
        define("BACKUP_DIR", $BACKUP_DIR); // Comment this line to use same script's directory ('.')
        define("TABLES", $TABLES); // Full backup
        //define("TABLES", 'table1, table2, table3'); // Partial backup
        define("CHARSET", $CHARSET);
        define("GZIP_BACKUP_FILE", $GZIP_BACKUP_FILE); // Set to false if you want plain SQL backup files (not gzipped)
        define("BATCH_SIZE", $BATCH_SIZE); // Batch size when selecting rows from database in order to not exhaust system memory
